...
- 单一可选主题不得在原生「外观」下再复制一套主题选择器；优先使用 DSH 官方 Slot shadowing 扩展 `appearance`，保持一个选择权威。
- 声明继承内置 Light/Dark 的主题必须完整复用对应调色板。若视觉只属于特定页面或背景层，主题 token 覆盖应保持为空，并将样式限制在插件自己的 DOM 标记下。
- 如确需修改全局 token，必须逐项说明用途，并在相同视口下与所继承的内置主题做设置页、侧边栏和通用控件的可视化对照。
